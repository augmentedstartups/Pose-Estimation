...
def draw_detection(
  ...
  cv2.rectangle(image, rect_start_point, rect_end_point,
                (255, 255, 255), bbox_drawing_spec.thickness)
...
def draw_landmarks(
  ...
  for landmark_px in idx_to_coordinates.values():
    if (counter % 2) == 0:
      cv2.circle(image, landmark_px, landmark_drawing_spec.circle_radius,(184, 182, 0), landmark_drawing_spec.thickness)
    else:
      cv2.circle(image, landmark_px, landmark_drawing_spec.circle_radius, (56, 116, 212), landmark_drawing_spec.thickness)
    counter += 1
